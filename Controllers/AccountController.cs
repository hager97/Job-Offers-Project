...
namespace jobs.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            else { ModelState.AddModelError("", "This Account is blocked"); return View(model); }
        }
        ...
        {  var u_ser = db.Users.FirstOrDefault(x => x.UserName == model.Email);
            if (ModelState.IsValid)
            { if (u_ser is null)
                {
                    ViewBag.UserType = new SelectList(db.Roles.Where(a => !a.Name.Contains("Admin")).ToList(),
                                                "Name", "Name");
                    var user = new ApplicationUser { UserName = model.UserName, Email = model.Email, UserType = model.UserType,Statues="متاح" };
        ...
        { 
            ...
            {       
                var newpass = UserManager.PasswordHasher.HashPassword(profile.NewPassword);
